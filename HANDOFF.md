...
**Flow State performance mission started 2026-07-11 on `perf/reliability-baseline`.** The sharpened goal is measurable competitor-leading responsiveness, verified control reliability, and ten evidence-backed differentiators; do not claim faster or exclusive without matching proof. Baseline: import 2314.4 ms, Moonshine load 4303.5 ms, warmed 0.5-second silent inference 29.1-52.6 ms, 10-second WAV/history save 11.6 ms, text-only history save 4.1 ms. First fix routes normal, continuous, and command transcripts through `deliver_text()`, which inserts before history persistence and catches ordinary storage failures. The new test failed on old code and full discovery is now 14 tests OK. Wargame: `wargames/flow-state-performance.md`. Next: build the median/p95 benchmark harness and Hub control-action matrix, then run live Notepad timing. Direct `py_compile` currently hits a locked `__pycache__` target; use a separate output path.
