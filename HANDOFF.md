...
**Flow State performance mission active 2026-07-11 on `perf/reliability-baseline`.** The sharpened goal is measurable competitor-leading responsiveness, verified control reliability, and ten evidence-backed differentiators; never claim faster or exclusive without matching proof. Shipped on branch: `641fa89` inserts before history persistence and survives storage errors; `0df4657` drops overlay polling 100 to 20 ms, lazy-loads audio in parallel, and adds full Hub command coverage; `ad624fd` adds `benchmark_flow.py` plus usage docs. Clean-process import+engine-warm median improved 4413.4 to 3416.2 ms (22.6%). Full desktop suite is 17 tests OK in 15.657s across all 9 Hub pages and 11 page-specific command buttons; 6 files compile. Latest quick benchmark: import median 604.8 ms/p95 790.4, text finish median 0.2/p95 0.6, text history median 2.3, 10-second audio history median 3.8, overlay ceiling 20 ms. Tk tests must run outside sandbox (real Tcl/Tk 8.6.12). Wargame: `wargames/flow-state-performance.md`. Next: live Notepad stop-to-insert median/p95, then rank/spec ten differentiators from refreshed evidence.
