...
**Flow State performance mission active 2026-07-11 on `perf/reliability-baseline`.** Existing branch commits improve delivery, startup, overlay response, controls, and add `benchmark_flow.py`. New `DIFFERENTIATORS.md` defines ten community-backed gaps scoped against public Wispr Flow, Aqua Voice, and Superwhisper features. #5 Clipboard Shield is implemented: Windows clipboard sequence checks prevent stale restore over newer user/app data, and locked clipboard access falls back to direct typing. Bite proof: removing the sequence guard made `copy` run twice (`dictated`, then stale `before`) and failed; restored guard passed. Full desktop suite is 20 tests OK in 13.916s across all 9 Hub pages and 11 page-specific command buttons; 7 files compile. Current performance proof remains 22.6% faster median import+engine-warm, 20 ms overlay ceiling, latest quick import median 604.8 ms. Live Notepad launch was temporarily blocked by desktop execution-service usage limits; do not replace it with a synthetic claim. Next: #1 Crash Journal, then #2 Recovery Inbox; retry real Notepad timing when launch access returns.
